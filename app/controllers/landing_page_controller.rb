class LandingPageController < ApplicationController
  ...
  def map_location
    ...
    @hash = Gmaps4rails.build_markers(@time_and_places) do |time_and_place, marker|
      ...
      marker.infowindow '<b class="infowindow_truck_name"><a href="/trucks/'+ time_and_place.truck.id.to_s + '">'+ time_and_place.truck.name+'</b></a>'  + "<b><br> Cuisine:</b> " + time_and_place.truck.cuisine + "<br>" +
  ...
  def get_markers_by_address
    ...
    @hash = Gmaps4rails.build_markers(@time_and_places) do |time_and_place, marker|
      marker.lat time_and_place.latitude
      marker.lng time_and_place.longitude
      marker.infowindow '<b><a href="/trucks/'+
      time_and_place.truck.id.to_s + '">'
      + time_and_place.truck.name+'
      </b></a>'  + "<b><br> Cuisine:</b> " +
      time_and_place.truck.cuisine + "<br>" +
      "<b> Menu: </b>" +
      time_and_place.truck.menu + 
      "<br><br><em>" +'<b>'+
      time_and_place.start_time.strftime("%a %m/%d") +
      "</b></em> <br>"'<b>Address: </b>' +
      time_and_place.street_address + ", "
      + time_and_place.city +
      '<br>' "<b><br> Start Time: </b>" +
       time_and_place.start_time.strftime("%I:%M %p") +
       '</b>' +"<b><br> End Time: </b>" +
       time_and_place.end_time.strftime("%I:%M %p")
      end
    render json: @hash.to_json
  end
